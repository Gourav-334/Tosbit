...
void makeDb()
{
	...
	if (checkDbExistence(FALSE)==TRUE)
	{
		...
		{
			clearEntity("directory");
			snprintf(directory, sizeof(directory), "cd data && rmdir /s /q %s", database);
...
void deleteTable()
{
	...
	else if (checkDbExistence(FALSE)==TRUE && checkTableExistence(FALSE)==TRUE)
	{
		...
		clearEntity("directory");
		snprintf(directory, sizeof(directory), "rmdir /s /q data\\%s\\%s", database, table);
		...
		{	
			...
			if (!strcmp(table,buffer))
			{
				...
				system("rmdir /s /q data\\cache.tosbit");
...
void deleteDb()
{
	...
	else if (checkDbExistence(FALSE)==TRUE)
	{
		...
		clearEntity("directory");
		snprintf(directory, sizeof(directory), "rmdir /s /q data\\%s", database);
